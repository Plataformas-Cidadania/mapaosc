...
public class FormularioWidget extends Composite {
	...
	private HTML getHTML(OrganizationModel org, Boolean editable) {
		...
		htmlBuilder.append("			<img src='imagens/osc/" + (org.getGoogle() != ""
				? org.getGoogle() + "' alt='" + org.getRazaoSocial() + "' width='160' height='160'"
				: "org_indisponivel.jpg'") + " />");
		htmlBuilder.append("		</div>");
		htmlBuilder.append("		<div class='redes'>");
		htmlBuilder.append("			<a href='"
				+ (org.getGoogle() != "" ? org.getGoogle() : "#O" + org.getId().toString())
				+ "'><button type='button' id='b-google' name='redes' title='Google' class='tooltip'></button></a>");
		htmlBuilder.append("			<a href='"
				+ (org.getFacebook() != "" ? org.getGoogle() : "#O" + org.getId().toString())
				+ "'><button type='button' id='b-facebook' name='redes' title='Facebook' class='tooltip'></button></a>");
		htmlBuilder.append("			<a href='"
				+ (org.getLinkedin() != "" ? org.getGoogle() : "#O" + org.getId().toString())
				+ "'><button type='button' id='b-linkedin' name='redes' title='LinkedIn' class='tooltip' ></button></a>");
		htmlBuilder.append("			<a href='"
				+ (org.getTwitter() != "" ? org.getGoogle() : "#O" + org.getId().toString())
				+ "'><button type='button' id='b-twitter' name='redes' title='Twitter' class='tooltip'></button></a>");
		htmlBuilder.append("		</div>");
		htmlBuilder.append("		<a href='"
				+ (org.getComoParticipar() != "" ? org.getComoParticipar() : "#O" + org.getId().toString())
				+ "' class='participar box'>Como Participar</a>");
		htmlBuilder.append("		<div class='recomendacoes'>");
		htmlBuilder.append("			<span class='tooltip' title='" + org.getRecomendacoes() + " recomendações'>"
				+ org.getRecomendacoes() + "</span>");
		htmlBuilder.append("		</div>");
		// htmlBuilder.append(" <button type='button'
		// class='recomendar'>Recomendar</button>");
		// htmlBuilder.append(" <div class='classificacao'>");
		// htmlBuilder.append(" <h3>Classificação</h3>");
		// htmlBuilder.append(" <ul class='dados'>");
		// htmlBuilder.append(" <li class='fasfil'><a href='#'>Educação e
		// pesquisa</a></li>");
		...
		// htmlBuilder.append(" <input type='text' name='razao_social'
		// id='razao_social' placeholder='Informação não disponível' value='" +
		// org.getRazaoSocial() + "' " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_oficial' title='Dado Oficial, Fonte RAIS'></span>");
		htmlBuilder.append("					</div>");
		htmlBuilder.append("					<div>");
		htmlBuilder.append("						<strong>Nome Fantasia:</strong>");
		htmlBuilder
				.append("						<input type='text' name='nome_fantasia' id='nome_fantasia' placeholder='Informação não disponível' value='"
						+ org.getNomeFantasia() + "' " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_oficial' title='Dado Oficial, Fonte RAIS'></span>");
		htmlBuilder.append("					</div>");
		htmlBuilder.append("					<div>");
		...
		// htmlBuilder.append(" <input type='text' name='cnpj' id='cnpj'
		// placeholder='Informação não disponível' value='" +
		// convertNumberToString(org.getCnpj()) + "' " + (editable ? "" :
		// "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_oficial' title='Dado Oficial, Fonte RAIS'></span>");
		htmlBuilder.append("					</div>");
		...
		if (org.getEndereco() == null || org.getEndereco() == "") {
			htmlBuilder.append(
					"						<span id='endereco' title='Endereço'>Informação não disponível</span>");
		} else {
			htmlBuilder.append("						<a href='" + GWT.getHostPageBaseURL() + "Map.html#O"
					+ org.getId().toString() + "'><span id='endereco' title='Endereço'>" + org.getEndereco()
					+ "</span></a>");
		}
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_oficial' title='Dado Oficial, Fonte RAIS'></span>");
		...
		// htmlBuilder.append(" <a href='Lista.html?grupoOSC=123'>visualizar
		// Lista</a>");
		// htmlBuilder.append(" <span class='fonte_de_dados dado_oficial'
		// title='Dado Oficial, Fonte RAIS'></span>");
		// htmlBuilder.append(" </div>");
		htmlBuilder.append("					<div>");
		...
		// htmlBuilder.append(" <input type='text' name='natureza_juridica'
		// id='natureza_juridica' placeholder='Informação não disponível'
		// value='" + org.getNaturezaJuridica() + "' " + (editable ? "" :
		// "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_oficial' title='Dado Oficial, fonte RAIS'></span>");
		htmlBuilder.append("					</div>");
		...
		// htmlBuilder.append(" <input type='text' name='cnae' id='cnae'
		// placeholder='Informação não disponível' value='" + org.getCnae() + "'
		// " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"					<span class='fonte_de_dados dado_oficial' title='Dado Oficial, fonte RAIS'></span>");
		htmlBuilder.append("					</div>");
		...
		// htmlBuilder.append(" <input type='text' name='responsavel'
		// id='responsavel' placeholder='Informação não disponível' value='" +
		// org.getResponsavel() + "' " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_oficial' title='Dado Oficial, fonte RAIS'></span>");
		htmlBuilder.append("					</div>");
		htmlBuilder.append("					<div>");
		...
		htmlBuilder
				.append("						<textarea name='descricao_projeto' id='descricao_projeto' placeholder='Informação não disponível' "
						+ (editable ? "" : "readonly") + ">" + org.getDescricaoProjeto() + "</textarea>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_organizacao' title='Dado preenchido pela Organização'></span>");
		htmlBuilder.append("					</div>");
		htmlBuilder.append("					<div>");
		htmlBuilder.append("						<strong>Ano de Fundação:</strong>");
		htmlBuilder
				.append("						<input type='number' name='ano_fundacao' id='ano_fundacao' placeholder='Informação não disponível' value='"
						+ convertNumberToString(org.getAnoFundacao()) + "' " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_organizacao' title='Dado preenchido pela Organização'></span>");
		htmlBuilder.append("					</div>");
		htmlBuilder.append("					<div>");
		...
		htmlBuilder
				.append("						<input type='text' name='site' id='site' placeholder='Informação não disponível' value='"
						+ org.getSite() + "' " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_organizacao' title='Dado preenchido pela Organização'></span>");
		htmlBuilder.append("					</div>");
		htmlBuilder.append("					<div>");
		...
		htmlBuilder
				.append("						<input type='text' name='email' id='email' placeholder='Informação não disponível' value='"
						+ org.getEmail() + "' " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_organizacao' title='Dado preenchido pela Organização'></span>");
		htmlBuilder.append("					</div>");
		...
		// htmlBuilder.append("<div class='botoes'><button type='button'
		// class='excluir participacao'>Excluir</button></div>");
		// htmlBuilder.append("<span class='fonte_de_dados dado_organizacao'
		// title='Dado preenchido pela Organização'></span>");
		// htmlBuilder.append("</li>");
		// htmlBuilder.append("<li id='addsocial' class='botoes'><button
		// type='button' class='adicionar
		// participacao'>Adicionar</button></li>");
		// }else{
		// for (String s : org.getParticipacaoSocial()) {
		// htmlBuilder.append(" <li>");
		// htmlBuilder.append(" <span class='visualizacao_dados'>" + s +
		// "</span>");
		// htmlBuilder.append(" <span class='fonte_de_dados dado_organizacao'
		// title='Dado preenchido pela Organização'></span>");
		// htmlBuilder.append(" </li>");
		...
		// title='Dado preenchido pela Organização'></span>");
		// htmlBuilder.append("</li>");
		// htmlBuilder.append("<li id='addRecursos' class='botoes'><button
		// type='button' class='adicionar
		// participacao'>Adicionar</button></li>");
		// }else{
		...
		htmlBuilder.append(
				" <h2>Quadro de Diretores <button type='button' class='adicionar' id='addDiretor' style='float:right;margin-top:3px;margin-right: 3px;'>Adicionar</button></h2>");
		htmlBuilder.append(" <fieldset>");
		...
		if (org.getDiretores().size() == 0) {
			...
			for (DiretorModel d : org.getDiretores()) {
				...
				htmlBuilder.append("<input type='text' name='" + d.getId() + "' id='cargo" + dir + "' value='"
						+ d.getCargo() + "'  " + (editable ? "" : "readonly") + " />");
				htmlBuilder.append("<input type='text' name='" + d.getId() + "' id='nome" + dir + "' value='"
						+ d.getNome() + "'  " + (editable ? "" : "readonly") + " />");
				htmlBuilder.append("<div class='botoes'><button id='removedir" + dir + "' value='" + d.getId()
						+ "' type='button' class='excluir participacao'>Excluir</button></div>");
				htmlBuilder.append(" </div>");
			}
		}
		htmlBuilder.append(" </div>");
		...
		if (org.getTotalColaboradores() == null || org.getTotalColaboradores() == -1)
			...
			htmlBuilder.append(
					"						<span>" + convertNumberToString(org.getTotalColaboradores()) + "</span>");
		// htmlBuilder.append(" <input type='text' name='total_colaboradores'
		// id='total_colaboradores' placeholder='Informação não disponível'
		// value='" + convertNumberToString(org.getTotalColaboradores()) + "' "
		// + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_oficial' title='Dado Oficial, Fonte RAIS'></span>");
		htmlBuilder.append(
				"						<div id='recolherCol' class='collapse-click collapse-button' data-toggle='collapse' data-target='#recCol' ><div></div></div>");
		htmlBuilder.append("					</div>");
		...
		if (org.getTrabalhadores() == null || org.getTrabalhadores() == -1)
			...
			htmlBuilder.append(
					"						<span>" + convertNumberToString(org.getTrabalhadores()) + "</span>");
		// htmlBuilder.append(" <input type='text' name='trabalhadores'
		// id='trabalhadores' placeholder='Informação não disponível' value='" +
		// convertNumberToString(org.getTrabalhadores()) + "' " + (editable ? ""
		// : "readonly") + "/>");
		htmlBuilder.append("						</div>");
		htmlBuilder.append("						<div>");
		htmlBuilder.append("							<strong>Voluntários:</strong>");
		htmlBuilder
				.append("							<input type='number' name='voluntarios' id='voluntarios' placeholder='Informação não disponível' value='"
						+ convertNumberToString(org.getVoluntarios()) + "' " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append("						</div>");
		htmlBuilder.append("						<div>");
		...
		if (org.getPortadoresDeficiencia() == null || org.getPortadoresDeficiencia() == -1)
			...
			htmlBuilder.append("						<span>" + convertNumberToString(org.getPortadoresDeficiencia())
					+ "</span>");
		// htmlBuilder.append(" <input type='text'
		// name='colaboradores_portadores_deficiencia'
		// id='colaboradores_portadores_deficiencia' placeholder='Informação não
		// disponível' value='" +
		// convertNumberToString(org.getPortadoresDeficiencia()) + "' " +
		// (editable ? "" : "readonly") + "/>");
		htmlBuilder.append("						</div>");
		htmlBuilder.append("					</div>");
		htmlBuilder.append("				</div>");
		...
		if (org.getValorParceriasTotal() == null || org.getValorParceriasTotal() == -1.0)
			...
			htmlBuilder.append("						<span>"
					+ convertNumberToCurrencyString(org.getValorParceriasTotal()) + "</span>");
		// htmlBuilder.append(" <input type='text' name='valor_total_parcerias'
		// id='valor_total_parcerias' placeholder='Informação não disponível'
		// value='" +
		// convertNumberToCurrencyString(org.getValorParceriasTotal()) + "' " +
		// (editable ? "" : "readonly") + "/>");
		htmlBuilder.append(
				"						<span class='fonte_de_dados dado_oficial' title='Dado Oficial, Fonte SINAFI'></span>");
		htmlBuilder.append(
				"						<div id='recolherParc' class='collapse-click collapse-button' data-toggle='collapse' data-target='#recParc' ><div></div></div>");
		htmlBuilder.append("					</div>");
		...
		if (org.getValorParceriasFederal() == null || org.getValorParceriasFederal() == -1.0)
			...
			htmlBuilder.append("						<span>"
					+ convertNumberToCurrencyString(org.getValorParceriasFederal()) + "</span>");
		// htmlBuilder.append(" <input type='text'
		// name='valor_parcerias_federais' id='valor_parcerias_federais'
		// placeholder='Informação não disponível' value='" +
		// convertNumberToCurrencyString(org.getValorParceriasFederal()) + "' "
		// + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append("						</div>");
		htmlBuilder.append("						<div>");
		...
		if (org.getValorParceriasEstadual() == null || org.getValorParceriasEstadual() == -1.0)
			...
			htmlBuilder.append("						<span>"
					+ convertNumberToCurrencyString(org.getValorParceriasEstadual()) + "</span>");
		// htmlBuilder.append(" <input type='text'
		// name='valor_parcerias_estaduais' id='valor_parcerias_estaduais'
		// placeholder='Informação não disponível' value='" +
		// convertNumberToCurrencyString(org.getValorParceriasEstadual()) + "' "
		// + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append("						</div>");
		htmlBuilder.append("						<div>");
		...
		if (org.getValorParceriasMunicipal() == null || org.getValorParceriasMunicipal() == -1.0)
			...
			htmlBuilder.append("						<span>"
					+ convertNumberToCurrencyString(org.getValorParceriasMunicipal()) + "</span>");
		// htmlBuilder.append(" <input type='text'
		// name='valor_parcerias_municipais' id='valor_parcerias_municipais'
		// placeholder='Informação não disponível' value='" +
		// convertNumberToCurrencyString(org.getValorParceriasMunicipal()) + "'
		// " + (editable ? "" : "readonly") + "/>");
		htmlBuilder.append("						</div>");
		htmlBuilder.append("					</div>");
		htmlBuilder.append("				</div>");
		...
		// htmlBuilder.append(" <h2>Últimas atualizações <a
		// href='atualizacoes.html' class='box'>Ver todas</a></h2>");
		// htmlBuilder.append(" <div>");
		// htmlBuilder.append(" <ul>");
		// htmlBuilder.append(" <li>Recebeu <a href='#'>10 novas
		// recomendações</a> <em class='data'>6 set 2013</em></li>");
		// htmlBuilder.append(" <li>Adicionou o projeto <a href='#'>Lorem Ipsum
		// dolor sit amet</a> <em class='data'>5 set 2013</em></li>");
		// htmlBuilder.append(" <li>Recebeu <a href='#'>50 novas
		// recomendações</a><em class='data'>20 jul 2013</em></li>");
		// htmlBuilder.append(" </ul>");
		// htmlBuilder.append(" </div>");
		...
		// htmlBuilder.append(" <li><a href='#'>Quadro de dirigentes</a> (210
		// KB)</li>");
		// htmlBuilder.append(" <li><a href='#'>Convênios</a> (1,3 MB)</li>");
		// htmlBuilder.append(" <li><a href='#'>Prestação de contas</a> (940
		// KB)</li>");
		...
		if (org.getCertificacao().size() == 0) {
			...
			for (String s : org.getCertificacao()) {
				...
				// htmlBuilder.append(" <li class='fonte'><em>Fonte:</em>
				// SICONV</li>");
			}
		}
		htmlBuilder.append("				</ul>");
		...
		htmlBuilder.append(
				"					<button id='addProjetos' type='button' class='adicionar'>Adicionar</button>");
		...
		if (org.getProjetos().size() == 0 && org.getConvenios().size() == 0) {
			...
			for (ProjetoModel p : org.getProjetos()) {
				...
				htmlBuilder
						.append("					<div id='recolherProj' class='collapse-click collapse-button' data-toggle='collapse' data-target='#recProj"
								+ proj + "' ><div></div></div>");
				htmlBuilder.append("						<div class='nome'>");
				htmlBuilder.append("							<span class='no_margin_right'>"
						+ (org.getProjetos().indexOf(p) + 1) + " |</span>");
				if (p.getTitulo() == null)
					htmlBuilder
							.append("							<span class='no_margin titulo_projeto'><input type='text' name='"
									+ p.getId() + "' id='projeto_nome_projeto" + proj
									+ "' placeholder='Informação não disponível' value='' "
									+ (editable ? "" : "readonly") + "/></span>");
				else
					htmlBuilder
							.append("							<span class='no_margin titulo_projeto'><input type='text' name='"
									+ p.getId() + "' id='projeto_nome_projeto" + proj
									+ "' placeholder='Informação não disponível' value='" + p.getTitulo() + "' "
									+ (editable ? "" : "readonly") + "/></span>");
				htmlBuilder.append("						</div>");
				...
				htmlBuilder
						.append("								<strong class='separador left-radius'>Status</strong>");
				...
				htmlBuilder
						.append("								<p><input type='hidden' name='projeto_data_inicio' id='projeto_data_inicio"
								+ proj + "' placeholder='Informação não disponível' value='"
								+ convertDateToString(p.getDataInicio()) + "' " + (editable ? "" : "readonly")
								+ "/></p>");
				htmlBuilder.append("							</div>");
				htmlBuilder.append("							<div class='col1_6'>");
				...
				htmlBuilder
						.append("								<p><input type='hidden' name='projeto_data_final' id='projeto_data_final"
								+ proj + "' placeholder='Informação não disponível' value='"
								+ convertDateToString(p.getDataFim()) + "' " + (editable ? "" : "readonly") + "/></p>");
				htmlBuilder.append("							</div>");
				htmlBuilder.append("							<div class='col1_6'>");
				htmlBuilder
						.append("								<strong class='separador'>Valor Total (R$)</strong>");
				htmlBuilder
						.append("								<p><input type='text' name='projeto_valor_total' id='projeto_valor_total"
								+ proj + "' placeholder='Informação não disponível' value='"
								+ convertNumberToCurrencyString(p.getValorTotal()) + "'  "
								+ (editable ? "" : "readonly") + " /></p>");
				htmlBuilder.append("							</div>");
				...
				if (editable == false) {
					if (p.getLink() == "link para página" || p.getLink() == "" || p.getLink() == null)
						...
						htmlBuilder.append("<p>");
						htmlBuilder.append("<span id='linkproj" + proj
								+ "' title='' class='menuTooltip'><span>Você será redirecionado para a página do Projeto</span>");
						htmlBuilder.append("<a id='link" + proj + "' href='http://" + p.getLink() + "' target='_blank' "
								+ (editable ? "contenteditable='true'" : "contenteditable='false'")
								+ " contenteditable='true' >link para página</a>");
						htmlBuilder.append("</span>");
						htmlBuilder.append("</p>");
					}
				} else {
					if (p.getLink() == null) {
						htmlBuilder.append("<p><input id='link" + proj + "' name='link" + proj
								+ "' placeholder='Digite a página do projeto' /></p>");
					} else {
						htmlBuilder.append("<p><input id='link" + proj + "' name='link" + proj + "' value='"
								+ p.getLink() + "' /></p>");
					}
				}
				htmlBuilder.append("							</div>");
				...
				htmlBuilder.append(
						"								<strong class='separador left-radius'>Público Alvo do Projeto</strong>");
				if (p.getPublicoAlvo() == null)
					htmlBuilder
							.append("								<textarea name='projeto_publico_alvo' id='projeto_publico_alvo"
									+ proj + "'  placeholder='Informação não disponível' "
									+ (editable ? "" : "readonly") + "></textarea>");
				else
					htmlBuilder
							.append("								<textarea name='projeto_publico_alvo' id='projeto_publico_alvo"
									+ proj + "'  placeholder='Informação não disponível' "
									+ (editable ? "" : "readonly") + ">" + p.getPublicoAlvo() + "</textarea>");
				htmlBuilder.append("							</div>");
				// htmlBuilder.append(" <div class='col1_6'>");
				// htmlBuilder.append(" <strong
				// class='separador'>Abrangência</strong>");
				// htmlBuilder.append(" <p>");
				// htmlBuilder.append(" <select " + (editable ? "" :
				// "disabled='disabled'") + ">");
				// htmlBuilder.append(" <option value='nacional' " +
				// (p.getAbrangencia() == "nacional" ? "checked='checked'" : "")
				// + "> Nacional </option>");
				// htmlBuilder.append(" <option value='regional' " +
				// (p.getAbrangencia() == "regional" ? "checked='checked'" : "")
				// + "> Regional </option>");
				// htmlBuilder.append(" <option value='estadual' " +
				// (p.getAbrangencia() == "estadual" ? "checked='checked'" : "")
				// + "> Estadual </option>");
				// htmlBuilder.append(" <option value='municipal' " +
				// (p.getAbrangencia() == "municipal" ? "checked='checked'" :
				// "") + "> Municipal </option>");
				// htmlBuilder.append(" </select>");
				// htmlBuilder.append(" </p>");
				// htmlBuilder.append(" </div>");
				// htmlBuilder.append(" <div class='col1_2'>");
				// htmlBuilder.append(" <strong class='right-radius'>Localização
				// do Projeto</strong>");
				// htmlBuilder.append(" <div class='localizacao_projet'>");
				// htmlBuilder.append(" <ul class='locais'>");
				// htmlBuilder.append(" <li><a
				// href='mapa.html#5'>Nordeste</a></li>");
				// htmlBuilder.append(" </ul>");
				// htmlBuilder.append(" </div>");
				// htmlBuilder.append(" </div>");
				htmlBuilder.append("						</div>");
				...
				htmlBuilder.append("							<div class='col1_3'>");
				htmlBuilder.append(
						"								<strong class='left-radius separador'>Financiadores do Projeto</strong>");
				if (p.getFinanciadores() == null)
					htmlBuilder
							.append("								<textarea name='financiadores' id='financiadores"
									+ proj + "'  placeholder='Informação não disponível' "
									+ (editable ? "" : "readonly") + "></textarea>");
				else
					htmlBuilder
							.append("								<textarea name='financiadores' id='financiadores"
									+ proj + "'  placeholder='Informação não disponível' "
									+ (editable ? "" : "readonly") + ">" + p.getFinanciadores() + "</textarea>");
				htmlBuilder.append("							</div>");
				htmlBuilder.append("							<div class='col2_3'>");
				htmlBuilder.append(
						"								<strong class='right-radius'>Descrição do Projeto</strong>");
				if (p.getDescricao() == null)
					htmlBuilder.append("								<textarea name='descprojeto' id='descprojeto"
							+ proj + "'  placeholder='Informação não disponível' " + (editable ? "" : "readonly")
							+ "></textarea>");
				else
					htmlBuilder.append("								<textarea name='descprojeto' id='descprojeto"
							+ proj + "'  placeholder='Informação não disponível' " + (editable ? "" : "readonly") + ">"
			...
			for (ConvenioModel c : org.getConvenios()) {
				conv++;
				count = proj + conv;
				String link = "https://www.convenios.gov.br/siconv/ConsultarProposta/ResultadoDaConsultaDeConvenioSelecionarConvenio.do?sequencialConvenio="
						+ c.getNConv() + "&Usr=guest&Pwd=guest";
				htmlBuilder.append("			<div class='clearfix projeto collapsable'>");
				htmlBuilder.append("				<div class='projeto_header'>");
				htmlBuilder
						.append("					<div id='recolherConv' class='collapse-click collapse-button' data-toggle='collapse' data-target='#recConv"
								+ conv + "' ><div></div></div>");
				htmlBuilder.append("						<div class='nome'>");
				...
				htmlBuilder.append("								<strong class='separador'>Data Início</strong>");
				htmlBuilder
						.append("								<p><span name='projeto_data_inicio' id='convenio_data_inicio' >"
								+ convertDateToString(c.getDataInicio()) + "</span></p>");
				htmlBuilder.append("							</div>");
				htmlBuilder.append("							<div class='col1_6'>");
				htmlBuilder.append("								<strong class='separador'>Data Final</strong>");
				htmlBuilder
						.append("								<p><span name='projeto_data_final' id='convenio_data_final' >"
								+ convertDateToString(c.getDataFim()) + "</span></p>");
				htmlBuilder.append("							</div>");
				htmlBuilder.append("							<div class='col1_6'>");
				htmlBuilder.append("								<strong class='separador'>Valor Total</strong>");
				htmlBuilder
						.append("								<p><span name='projeto_valor_total' id='convenio_valor_total' > R$ "
								+ convertNumberToCurrencyString(c.getValorTotal()) + "</span></p>");
				htmlBuilder.append("							</div>");
				...
				// htmlBuilder.append("<span>Informação não disponível</span>");
				// else
				// htmlBuilder.append("<span>"+ c.getFonteRecursos()
				// +"</span>");
				htmlBuilder.append("</div>");
				htmlBuilder.append("							<div class='col1_6'>");
				htmlBuilder.append("								<strong class='right-radius'>Link</strong>");
				htmlBuilder.append("<p>");
				htmlBuilder.append("<span id='linkconv" + conv
						+ "' title='' class='menuTooltip'><span>Você será redirecionado para a página SICONV. Nesta página são apresentados estatuto, quadro de diretores, instrumento do convênio e prestação de contas</span>");
				...
				if (c.getPublicoAlvo() == null)
					htmlBuilder.append("								<textarea name='" + c.getNConv()
							+ "' id='convenio_publico_alvo" + conv + "'  placeholder='Informação não disponível' "
							+ (editable ? "" : "readonly") + "></textarea>");
				else
					htmlBuilder.append("								<textarea name='" + c.getNConv()
							+ "' id='convenio_publico_alvo" + conv + "'  placeholder='Informação não disponível' "
							+ (editable ? "" : "readonly") + ">" + c.getPublicoAlvo() + "</textarea>");
				...
				// "disabled='disabled'") + ">");
				// htmlBuilder.append(" <option value='nacional'
				// checked='checked'> Nacional </option>");
				// htmlBuilder.append(" <option value='regional'
				// checked='checked'> Regional </option>");
				// htmlBuilder.append(" <option value='estadual'
				// checked='checked'> Estadual </option>");
				// htmlBuilder.append(" <option value='municipal'
				// checked='checked'> Municipal </option>");
				// htmlBuilder.append(" </select>");
				// htmlBuilder.append(" </p>");
				// htmlBuilder.append(" </div>");
				// htmlBuilder.append("<div class='col1_2'>");
		...
		if (editable) {
			...
			htmlBuilder.append("<a href='" + GWT.getHostPageBaseURL() + "Map.html#O" + org.getId().toString()
					+ "' >Cancelar</a> ou <input id='btnSalvar' type='submit' value='Salvar' class='salvar' />");
			htmlBuilder.append("</div>");
		}
