		...
		try {
			for(AgreementLocalityModel convenio : convenioList){
				...
				if(idOSC > 0){
					...
					pstmt = conn.prepareStatement(sql);
					pstmt.setLong(1, convenio.getCnpjProponente());
					pstmt.setString(2, convenio.getRazaoSocialProponente());
					pstmt.setString(3, convenio.getNomeFantasiaProponente());
					pstmt.execute();
					pstmt.close();
				...
				pstmt.setString(9, convenio.getOrgaoConcedente());
				pstmt.setString(10, convenio.getMunicipioProponente());
				pstmt.setString(11, convenio.getEnderecoProponente());
				pstmt.setString(12, convenio.getObjetoParceria());
				pstmt.execute();
				pstmt.close();
			}
