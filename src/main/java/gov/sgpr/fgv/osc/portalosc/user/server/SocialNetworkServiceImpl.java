...
public class SocialNetworkServiceImpl extends RemoteServiceImpl implements
	...
	public void addUser(FacebookUser user) throws RemoteException {
		...
		try {
			...
			pstmt2.setString(1, SafeHtmlUtils.htmlEscape(StringUtils.defaultString(user.getUid())));
			pstmt2.setInt(2, 2);
			pstmt2.setString(3, SafeHtmlUtils.htmlEscape(StringUtils.defaultString(user.getAccessToken())));
			pstmt2.setString(4, SafeHtmlUtils.htmlEscape(StringUtils.defaultString(user.getEmail())));
			pstmt2.execute();
...
//		}
	...
	public boolean hasUser(FacebookUser user) throws RemoteException {
		...
		try {
			...
			pstmt.setString(1, SafeHtmlUtils.htmlEscape(StringUtils.defaultString(user.getUid())));
